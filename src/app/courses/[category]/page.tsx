...
export default async function CoursePage({params: {category}}: CourseParams) {
  ...
  return (
    ...
    <section className="relative py-20 px-5 bg-white overflow-hidden">
      <div className="absolute left-[20%] sm:left-[40%] top-[1rem] rotate-[-45deg] rounded-t-[4rem] rounded-b-none bg-gradient-to-br from-primary-400 to-pink-200 overflow-hidden origin-top-left border-2 border-slate-500 w-full h-[200%]">
      ...
      <div className="container relative mx-auto max-w-[980px] px-4 flex flex-col">
        <div className="sm:w-[80%] md:w-[65%] lg:w-[50%] bg-white/50 md:bg-transparent pt-6 pr-3 flex flex-col justify-center">
          ...
          <h3 className="text-primary-500 text-3xl md:text-4xl font-bold">{category}</h3>
          ...
          <Link href="#courses" className="text-white bg-primary-500 rounded-[2rem] w-max px-5 py-2 text-sm md:text-md shadow-lg shadow-primary-200 my-2">See Courses</Link>
        </div>
      </div>
    </section>
    <section className="bg-gray-50 px-4">
      <div id="#courses" className="container relative mx-auto max-w-[980px] py-10 flex flex-col overflow-hidden">
        <h3 className="py-2 px-3 text-primary-500 border-0 border-l-[3px] border-l-primary-500 font-bold text-lg md:text-xl">{category} Courses</h3>
